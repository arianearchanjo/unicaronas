// frontend/js/notificacoes-ui.js
import { NotificacoesAPI } from './notificacoes-api.js';

export const NotificacoesUI = {
  isOpen: false,
  notificacoes: [],
  ...
  injectStyles() {
    ...
    style.textContent = `
      #uc-notif-container { position: relative; display: inline-flex; align-items: center; margin-right: 1rem; }
      #uc-notif-btn { background: transparent; border: none; color: var(--text); cursor: pointer; position: relative; padding: 8px; display: flex; transition: color 0.2s; }
      #uc-notif-btn:hover { color: var(--accent); }
      #uc-notif-badge { position: absolute; top: 4px; right: 4px; background: #ff4757; color: white; font-size: 10px; font-weight: bold; min-width: 16px; height: 16px; border-radius: 50%; display: none; align-items: center; justify-content: center; padding: 0 4px; border: 2px solid var(--surface); }
      #uc-notif-panel { position: fixed; top: 0; right: -350px; width: 350px; height: 100vh; background: var(--surface); border-left: 1px solid var(--border); box-shadow: -5px 0 25px rgba(0,0,0,0.3); z-index: 10000; transition: right 0.3s ease; display: flex; flex-direction: column; }
      #uc-notif-panel.open { right: 0; }
      .uc-notif-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.5); backdrop-filter: blur(2px); z-index: 9999; display: none; }
      .uc-notif-overlay.open { display: block; }
      .uc-notif-header { padding: 1.5rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; }
      .uc-notif-header h3 { margin: 0; font-size: 1.25rem; color: var(--text); }
      .uc-notif-close { background: none; border: none; color: var(--text-2); cursor: pointer; font-size: 1.5rem; }
      .uc-notif-list { overflow-y: auto; flex: 1; }
      .uc-notif-item { padding: 1rem 1.5rem; border-bottom: 1px solid var(--border); cursor: pointer; transition: background 0.2s; display: flex; gap: 1rem; position: relative; }
      .uc-notif-item:hover { background: rgba(255,255,255,0.03); }
      .uc-notif-item.nao-lida::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; background: var(--accent); }
      .uc-notif-icon { width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: var(--border); color: var(--text-2); }
  ...
  addEventListeners() {
    ...
    const togglePanel = async (open) => {
      ...
      if (this.isOpen) {
        await NotificacoesAPI.marcarComoLidas();
  ...
  async atualizar() {
    this.notificacoes = await NotificacoesAPI.listar();
    this.renderizar();
  },
  ...
  renderizar() {
    ...
    list.innerHTML = this.notificacoes.map(n => {
